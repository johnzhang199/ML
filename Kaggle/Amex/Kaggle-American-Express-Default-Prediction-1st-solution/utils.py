...
def NN_train_and_predict(train, test, model_class, config, use_series_oof, logit=False, output_root='./output/', run_id=None):
    ...
    if train is not None:
        ...
        for fold, (trn_index, val_index) in enumerate(skf.split(train_y,train_y[label_name])):
            ...
            train_dataloader = DataLoader(train_dataset,batch_size=batch_size ,shuffle=True, drop_last=True, collate_fn=train_dataset.collate_fn,num_workers=args.num_workers, pin_memory=True)
            ...
            if args.do_train:
                for epoch in range(epochs):
                    ...
                    valid_Y = train_y.iloc[val_index][label_name].values # oof train
            ...
            state_dict = torch.load(output_path + 'fold%s.ckpt'%fold, torch.device('cuda' if use_cuda() else 'cpu') )
            ...
            oof.iloc[val_index,oof.columns.get_loc(label_name)] = valid_preds
