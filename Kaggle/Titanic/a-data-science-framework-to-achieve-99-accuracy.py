...
data_raw = pd.read_csv(FOLDER + 'train.csv')


#a dataset should be broken into 3 splits: train, test, and (final) validation
#the test file provided is the validation file for competition submission
#we will split the train set into train and test data in future sections
data_val  = pd.read_csv(FOLDER + 'test.csv')
...
OH_cols_train = pd.DataFrame(OH_encoder.fit_transform(data1[['Pclass']]))
OH_columns = OH_encoder.get_feature_names_out()
OH_cols_train.columns = OH_columns

# One-hot encoding removed index; put it back
OH_cols_train.index = data1.index
...
#the following crashes on my pc
...
MLA = [
    ...
    #GLM
    linear_model.LogisticRegressionCV(),#classifier
    linear_model.PassiveAggressiveClassifier(),
    linear_model.RidgeClassifierCV(),
    linear_model.SGDClassifier(),
    linear_model.Perceptron(),#classifier
    
    #Navies Bayes
    naive_bayes.BernoulliNB(),#classifier
    naive_bayes.GaussianNB(),#classifier
    
    #Nearest Neighbor
    neighbors.KNeighborsClassifier(),
    
    #SVM, can be used in regression: https://towardsdatascience.com/support-vector-machine-introduction-to-machine-learning-algorithms-934a444fca47
    ...
    XGBClassifier()    #XGBRegressor is the regression model.
    ]
